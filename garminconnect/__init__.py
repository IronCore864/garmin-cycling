...
class Garmin:
    ...
    def download(self, path: str, **kwargs: Any) -> Any:
        """Wrapper for garth download with error handling and retry logic."""
        ...
        for attempt in range(1, _DOWNLOAD_MAX_RETRIES + 1):
            try:
                ...
                if status and 400 <= status < 500:
                    # Client errors are deterministic — do not retry
                    raise GarminConnectConnectionError(
                        f"Download client error ({status}): {e}"
                    ) from e

                # 5xx / other HTTP errors are transient — retryable
                exc = GarminConnectConnectionError(f"Download error: {e}")
                exc.__cause__ = e
                last_exc = exc
            except Exception as e:
                exc = GarminConnectConnectionError(f"Download error: {e}")
            ...
            if attempt < _DOWNLOAD_MAX_RETRIES:
                logger.warning(
                    ...
                    attempt, _DOWNLOAD_MAX_RETRIES, path, _DOWNLOAD_RETRY_DELAY,
                )
                time.sleep(_DOWNLOAD_RETRY_DELAY)
            else:
                logger.exception(
                    "Download failed for path '%s' after %d attempts", path, _DOWNLOAD_MAX_RETRIES
                )
    ...
    def get_max_metrics_date_range(self, start_date, end_date: str) -> dict[str, Any]:
        """Return available max metric data for 'cdate' format 'YYYY-MM-DD'."""
        start_date = _validate_date_format(start_date, "start_date")
        end_date = _validate_date_format(end_date, "end_date")
        url = f"{self.garmin_connect_metrics_url}/{start_date}/{end_date}"
        logger.debug("Requesting max metrics")
    ...
    def set_activity_note(self, activity_id: str, note: str) -> Any:
        """Set note/description for activity with id."""
        url = f"{self.garmin_connect_activity}/{activity_id}"
        payload = {"activityId": activity_id, "description": note}
